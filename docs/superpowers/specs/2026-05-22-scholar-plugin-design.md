...
**License compatibility.** The upstream is MIT-licensed (`bun pm view @modelcontextprotocol/server-pdf@1.7.2`, confirmed 2026-05-24); MIT permits vendoring and redistribution under scholar's own MIT license, subject to preservation of the upstream copyright notice. The license text and audit record live at `src/vendor/pdf-server/UPSTREAM-LICENSE`. Re-vendoring on a new upstream version must re-verify the license — see the open chore `upstream-pdf-server-revendor-process`. The full step-by-step re-vendor procedure (VERSION pin → helper → drift gates → license check → commit) is documented in `docs/vendor/pdf-server-revendor.md`.
...
### 7.2.1 The vendored sqlite-vec source

**Approach.** Vendor the `sqlite-vec` v0.1.9 amalgamation source under `src/vendor/sqlite-vec/` so the build pipeline's §14.1 step 5 compile-from-source fallback can compile against a known, version-pinned source tree when the prebuilt `vec0` binary's ABI does not match Bun's bundled SQLite engine. The npm package `sqlite-vec` continues to ship prebuilt binaries as the primary path — this vendoring only feeds the fallback. The version pin (v0.1.9) is deliberate: it must match the `sqlite-vec@0.1.9` entry in `bun.lock` so that the source compiled from this tree produces a `vec0` binary whose expected SQLite ABI agrees with the prebuilt binary (same codebase, same version, different compilation path).

**License compatibility.** sqlite-vec v0.1.9 is **dual-licensed Apache-2.0 / MIT** (author: Alex Garcia, Copyright © 2024 — confirmed via raw-tree `LICENSE-MIT` and `LICENSE-APACHE` at `https://raw.githubusercontent.com/asg017/sqlite-vec/v0.1.9/` on 2026-05-25). Both Apache-2.0 and MIT are permissive licenses fully compatible with scholar's own MIT license. Vendoring this source and redistributing it as part of the scholar `.plugin` archive is permitted, subject to preservation of the upstream copyright notice and license text. The complete audit record — source URLs, SHA-256 hashes for each vendored file, and license confirmation — lives at `src/vendor/sqlite-vec/UPSTREAM-AUDIT.md`.

**Why not rely on the npm package's prebuilts alone.** The npm-installed `node_modules/sqlite-vec/` tree is **not** shipped in the `.plugin` archive — only the prebuilt binary is bundled at build time. If a Bun upgrade shifts the bundled SQLite ABI after install, the prebuilt binary fails and the fallback (§14.1 step 5) must compile `vec0` from source. Without a vendored source tree, the fallback would have to reach out to the upstream at user-environment compile time, which is fragile and non-deterministic. By vendoring the source at a known path (`src/vendor/sqlite-vec/`) the build pipeline's fallback step has a stable, offline-available input that packaging can rely on without any additional download.

**Re-vendor.** When bumping to a newer sqlite-vec release, follow the re-vendor procedure documented in `src/vendor/sqlite-vec/UPSTREAM-AUDIT.md` — download the new amalgamation tarball from the GitHub release, re-compute SHA-256 hashes, update the audit record, and re-run the cycle 6.5 vec0 smoke test against the newly compiled or prebuilt binary to confirm ABI compatibility with the active Bun release.
...
4. `bun run build:runtime` — copies the Bun runtime binary (`bun.exe` on Windows, `bun` on POSIX) from the build host's Bun install to `build/runtime/`. Required because scholar's compiled binary cannot re-exec the Node-style pdf-child script under its embedded runtime (§7.2); this bundled `bun` is the spawn target. The runtime version pinned at build time is recorded in `package.json`'s `scholar.bundledBunVersion` field (the value of `bun --version` at build time). Both `scholar.bundledBunVersion` and `scholar.bunSqliteVersion` are read from the same Bun release — they record **different facts** about that release: `bundledBunVersion` is the Bun runtime version; `bunSqliteVersion` (step 5) is the SQLite library version that Bun's `bun:sqlite` module statically links against. Their string values are therefore **expected to differ** (e.g. `"1.3.11"` vs `"3.51.2"`). The `SCHOLAR_BUILD_MISMATCH` invariant fires when either field is missing or empty — not on string inequality between them.
5. `bun run build:vec` — produces the `vec0` shared library at `build/vendor/sqlite-vec/`. Default path: copy a prebuilt `vec0.dll` (Windows-x64, ABI-pinned to the SQLite version Bun's `bun:sqlite` links against in this Bun release — see §16). Fallback path: four specifics govern the compile-from-source path when the prebuilt and Bun's linked SQLite disagree on ABI. **(1) ABI-mismatch detection:** the build script attempts to load the prebuilt binary and run a one-row `vec0` schema creation; on failure it inspects the SQLite error string for patterns characteristic of ABI mismatch — specifically `"library used incorrectly"` or `"could not get library handle"` — and classifies those patterns as an ABI-mismatch trigger; generic load failures (file-missing, wrong path) are surfaced as user-environment errors and do NOT trigger the fallback compile. **(2) Compile invocation:** on ABI-mismatch detection, the script compiles `vec0` from the vendored source at `src/vendor/sqlite-vec/sqlite-vec.c` (§7.2.1), with the Bun runtime's `sqlite3.h` extracted into a scratch include directory; canonical command: `cc -O2 -fPIC -shared -I<bun-sqlite-include> src/vendor/sqlite-vec/sqlite-vec.c -o runtime/vendor/sqlite-vec/vec0.<ext>` where `<ext>` is `.dll` / `.so` / `.dylib` per host OS. **(3) Compiled-artifact cache:** the output lands at `runtime/vendor/sqlite-vec/vec0.<ext>` — NOT `build/vendor/...` — because the fallback runs at user-environment time (post-install Bun upgrade), not build-pipeline time; the artifact is cached and reused across scholar restarts, and only a re-detected ABI mismatch (rare) triggers a re-compile. **(4) C toolchain scope:** `cc` is a BUILD-ENVIRONMENT requirement (CI and `bun run build:vec`) — NOT a runtime user dependency for the common path; for v1, the `.plugin` ships an ABI-matched prebuilt produced at CI time, so the runtime fallback compile only fires on post-install Bun-upgrade ABI drift, which is rare and well-defined; if triggered on a user machine, a C toolchain is required and this is documented in packaging-006's README "Building from source" section (cycle 6.13).
