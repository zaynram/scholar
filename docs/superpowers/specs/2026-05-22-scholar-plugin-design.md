...
**License compatibility.** The upstream is MIT-licensed (`bun pm view @modelcontextprotocol/server-pdf@1.7.2`, confirmed 2026-05-24); MIT permits vendoring and redistribution under scholar's own MIT license, subject to preservation of the upstream copyright notice. The license text and audit record live at `src/vendor/pdf-server/UPSTREAM-LICENSE`. Re-vendoring on a new upstream version must re-verify the license — see the open chore `upstream-pdf-server-revendor-process`.
