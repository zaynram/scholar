...
| `corpus` | 6.3, 6.11 | Corpus and roots tools (`corpus.ts`, `roots.ts`, `snapshot.ts`), first-run PDF-root wizard (`scripts/first-run.ts`), (sqlite3-mcp registration removed post-posture-B 2026-05-24), `scholar.dashboard` view-opener. **The first-run wizard is entirely corpus-owned — packaging does not touch `scripts/first-run.ts`.** |
