...
| 3 | §7.6 frozen contracts (`ServerContext`/`PdfChild`/…) pinned for v1 | convention-only (one **deliberate amendment** landed — Δ7) | No automated guard. Honored, with one exception made by the spec owner in maintenance mode: `ServerContext` gained `runtimeRoot: string` via a deliberate spec edit (spec §7.6 amendment note), **not** a freeze allowance. The earlier pass *declined* this same edit (Δ2) to respect the freeze during execution; the freeze's purpose is now discharged. Pin: `index.test.ts` (literal sources from deps) + `corpus.test.ts` divergent-root dispatch (handler honors the field). See Δ7. |
...
| 6 | Single runtime-root source of truth | **was `convention-only` → now enforced-by-construction + enforced-in-test** | Δ2 collapsed `main()`'s inlined copy onto `resolveRuntimeRoot()`; Δ7 finished the job — `resolveRuntimeRoot()` now runs **once** in `main()`, flows in as `deps.runtimeRoot`, is captured on `ctx.runtimeRoot`, and the six corpus-handler sites read that field (zero per-call env reads). Pinned end-to-end: `index.test.ts` (literal sources from deps) + `corpus.test.ts` "writes the per-corpus DB under ctx.runtimeRoot, ignoring a divergent SCHOLAR_RUNTIME_ROOT" (handler honors it — proven to go red when a handler is reverted to an env read). See Δ2 + Δ7. |
...
- **Completed in Δ7 (was: deliberate refusal):** threading `runtimeRoot` through `ServerContext` so handlers stop calling `resolveRuntimeRoot()` directly — the "real" fix. It **mutates a §7.6 frozen contract**, so the first pass deferred it pending a deliberate spec edit rather than overriding the freeze silently. That edit landed 2026-06-04 (spec §7.6 amendment note + Δ7 below); the original scruple stands as written — this completes it via the proper unfreeze.
...
### Δ7 — INV-3/INV-6: `runtimeRoot` threaded through `ServerContext` (completes Δ2)
- **What:** `ServerContext` gained one field, `runtimeRoot: string` (`registry.ts`), populated once from `deps.runtimeRoot` at the sole typed ctx literal (`index.ts`). The six corpus-handler resolution sites (`corpus.ts`: `reopenPersistedCorpus` default param + `handleCreate`/`handleActivate`/`handleArchive`/`handleExport`/`handleResetInit`) now read `ctx.runtimeRoot` instead of each calling `resolveRuntimeRoot()`. After this, `resolveRuntimeRoot()` is *called* exactly once in production — `main()` (`index.ts`) — and only *defined/exported* in `corpus.ts`. This retires the dual resolution path Δ2 collapsed-but-could-not-finish.
- **Authority (the honest version):** this **mutates a §7.6 frozen contract**, which the first pass explicitly refused (Δ2) as out of maintenance-mode authority. It is licensed now **not** because the freeze permitted it — the freeze's "a downstream plan that needs more threads it through `ServerContext`, never by editing `registry.ts`" clause *barred* exactly this during execution — but by a **deliberate spec edit** under the project's "spec wins until a deliberate spec edit lands" rule. The freeze's only stated purpose (content-disjoint blast-radii so `worktree="not-required"` held across concurrent wave-2 execution) is fully discharged in maintenance mode: no concurrent plans remain to collide. Recorded in the spec §7.6 amendment note + the CLAUDE.md §7.6 line.
- **Behavior change:** none for the supported case (`main()` already resolved the same value and passed it as `deps.runtimeRoot`; every test aligns env with deps — see `corpus.test.ts` setup and `cli.test.ts:136-137`). Strictly *more* correct in the unsupported case: a mid-session `SCHOLAR_RUNTIME_ROOT` mutation can no longer split the config DB and a corpus DB onto different roots.
- **Pins (two links, non-bluffing):** the chain is *literal sources from deps → handler consumes the field*; one test per link.
  1. `index.test.ts` "populates ctx.runtimeRoot from deps, not from env" — deps and env deliberately **disagree**; asserts the *ctx literal* takes deps (catches a regression to `runtimeRoot: resolveRuntimeRoot()` in the literal). Narrow but real; does **not** by itself prove handler consumption.
  2. `corpus.test.ts` "writes the per-corpus DB under ctx.runtimeRoot, ignoring a divergent SCHOLAR_RUNTIME_ROOT" — the **strong tripwire**: builds with `deps=A`, sets `env=B` (B≠A), dispatches `scholar.corpus.create`, asserts the new DB lands under `A/dbs/…` and **not** `B`. **Proven discriminating** this pass — temporarily reverting `handleCreate` to `resolveRuntimeRoot()` landed the DB under B → red (the exact Δ2 regression); restored. Without this test the whole suite sets `env == deps`, so handler-reads-ctx and handler-reads-env are indistinguishable.
- Full suite `bun test src tests`: **350 pass / 0 fail (+2)**; `tsc --noEmit` clean.
...
- **§7.6 unfreeze (row 3): DONE — Δ7.** `runtimeRoot` threaded through `ServerContext` via a deliberate spec edit; dual resolution path retired; pinned by `index.test.ts` (literal) + `corpus.test.ts` divergent-root dispatch (handler, proven discriminating). The freeze otherwise still holds — no further `ServerContext` fields without the same deliberate-edit bar.
- **F3:** pid-reuse residual — accepted/documented (above), not selected.

*(INV-8 done — Δ4; INV-9, INV-10 done — Δ5; INV-1 §12.0 done — Δ6; INV-3/INV-6 §7.6 thread done — Δ7.)*
