...
- `bin/` + `dist/` — the slim-plugin launcher/provisioner scripts (`launch.mjs`, `ensure-bun.{sh,ps1}`, `scholar.nu`) and the built `server.js` + `pdf-server/` bundles.
...
- **scholar MCP server** — Bun + `bun:sqlite` + `drizzle-orm/bun-sqlite` + `sqlite-vec`, shipped as a `bun build --target=bun` bundle (`dist/server.js`, ~2.5 MB, **not** `--compile`). The `bun` runtime is **provisioned** into `${CLAUDE_PLUGIN_DATA}` by `bin/ensure-bun` at first launch (pinned to `scholar.bundledBunVersion` for the vec0 ABI), not shipped. A cross-OS launcher (`bin/launch.mjs`, manifest command `bun`) provisions-then-runs the server (M2 — SessionStart does not block MCP spawn). Exposes corpus, ingestion, annotation, digest, prompts, search, and UI-resource tools. Spawns the pdf MCP as a child.
...
- **§7.6 frozen contracts.** `ServerContext`, `PdfChild`, `ConfigAccessor`, `Logger`, `registerTools`, `runRawDdl` are pinned. `ctx.db` is *snapshot-at-entry* (a tool handler snapshots into a local on its first line; `corpus.activate` mutates `ctx.db` in place). Cross-plan helpers take a `tx` first arg; the Ollama client is a foundation singleton imported directly (not on `ServerContext`). **Maintenance amendment (2026-06-04):** `ServerContext` gained one field, `runtimeRoot: string`, by a *deliberate* spec edit (not a freeze allowance) now that the freeze's parallel-execution purpose is discharged — `main()` resolves the runtime root once and corpus handlers read `ctx.runtimeRoot` instead of re-calling `resolveRuntimeRoot()` per request, retiring the dual resolution path. The freeze otherwise still holds: do not add further `ServerContext` fields casually. See spec §7.6 amendment note + `docs/audits/2026-06-04-invariant-enforcement.md` Δ7.
