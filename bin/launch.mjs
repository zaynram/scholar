...
// provisioner's output is mapped to OUR stderr (the stdio integer `2` shares the
// parent's fd 2): stdout (fd 1) stays clean for the JSON-RPC stream the child
// bun will own.
...
// Built bundle (dist/server.js) wins if present; source-sync has no dist/, so it
// falls through to the TS entrypoint bun runs directly (auto-installing the
// reached import graph into BUN_INSTALL's cache on first call).
const entry = existsSync(join(root, "dist", "server.js"))
  ? join(root, "dist", "server.js")
  : join(root, "src", "server", "index.ts");

// ── 3. Cross-OS env defaults derived from the REAL env var ───────────────────
// CLAUDE_PLUGIN_DATA persists across plugin updates and is set on every host, so
// it is the correct cross-OS anchor for both the bun auto-install cache and
