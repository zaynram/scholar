...
import { buildServer, type BuiltServer } from "../src/server/index.ts";
import { openWithPragmas, applyMigrations } from "../src/server/db/migrations.ts";
import type { PdfChild } from "../src/server/tools/registry.ts";
import { runFirstRun } from "./first-run.ts"; // Will fail Red — not exported yet
...
test("runFirstRun returns FIRST_RUN_ELICIT_UNAVAILABLE when host lacks elicitation capability", async () => {
  ...
  const result = await runFirstRun(built.ctx, mockServer as unknown as import("@modelcontextprotocol/sdk/server/mcp.js").McpServer, dir);
