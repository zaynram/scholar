  ...
  return {
    ...
    pdf: {
      ...
      getText: async (viewUUID: string) =>
        typeof textFactory === "string" ? textFactory : await textFactory(viewUUID),
      currentRoots: () => ["/tmp"],
      setRoots: async () => {},
    ...
    log: noopLog,
    withCorpus: async <T,>(fn: (db: ReturnType<typeof drizzle>) => Promise<T> | T) =>
      await fn(db),
    embed: async (_model: string, prompt: string) => deterministicEmbedding(768, prompt),
...
test("refresh-extraction: writes paper_chunks AND chunk_vec rows for a fixture paper (count parity)", async () => {
  ...
  const n = (sqlite.query(
    "SELECT COUNT(*) AS n FROM paper_chunks WHERE paper_id='p1'",
  ).get() as { n: number }).n;
  expect(n).toBeGreaterThanOrEqual(2);

  // chunk_vec row count for this paper equals paper_chunks row count.
  const vN = (sqlite.query(
  ...
  const pend = (sqlite.query(
    "SELECT COUNT(*) AS n FROM paper_chunks WHERE embedded_at IS NULL",
  ).get() as { n: number }).n;
  expect(pend).toBe(0);

  // ULIDs: 26-char Crockford base32 (excludes I/L/O/U).
  const sample = (sqlite.query(
    "SELECT id FROM paper_chunks WHERE paper_id='p1' LIMIT 1",
  ).get() as { id: string }).id;
...
test("refresh-extraction: lazily materializes chunk_vec when deferred (via _testProbeDim)", async () => {
  ...
  const flag = (sqlite.query(
    "SELECT value FROM settings WHERE key='chunk_vec.created'",
...
test("refresh-extraction: idempotent — re-run yields equal counts (Ruling B count-based)", async () => {
  ...
  const n1 = (sqlite.query(
    "SELECT COUNT(*) AS n FROM paper_chunks WHERE paper_id='p3'",
  ).get() as { n: number }).n;
  const v1 = (sqlite.query(
    `SELECT COUNT(*) AS n FROM chunk_vec
     WHERE chunk_id IN (SELECT id FROM paper_chunks WHERE paper_id='p3')`,
  ).get() as { n: number }).n;

  await refreshExtraction(fakeCtx(sqlite), { paper_id: "p3" });
  const n2 = (sqlite.query(
    "SELECT COUNT(*) AS n FROM paper_chunks WHERE paper_id='p3'",
  ).get() as { n: number }).n;
  const v2 = (sqlite.query(
    `SELECT COUNT(*) AS n FROM chunk_vec
     WHERE chunk_id IN (SELECT id FROM paper_chunks WHERE paper_id='p3')`,
  ).get() as { n: number }).n;
...
test("refresh-extraction: ordinal shrinkage — fewer chunks on re-run trims stale paper_chunks + chunk_vec", async () => {
  ...
  await refreshExtraction(
    fakeCtx(sqlite, { text: "word ".repeat(1000) }),
    { paper_id: "p4" },
  );
  const longCount = (sqlite.query(
    "SELECT COUNT(*) AS n FROM paper_chunks WHERE paper_id='p4'",
  ).get() as { n: number }).n;
  expect(longCount).toBeGreaterThan(1);

  // Second run: tiny text → 1 chunk.
  await refreshExtraction(
    fakeCtx(sqlite, { text: "Tiny." }),
    { paper_id: "p4" },
  );
  const shortCount = (sqlite.query(
    "SELECT COUNT(*) AS n FROM paper_chunks WHERE paper_id='p4'",
  ).get() as { n: number }).n;
  expect(shortCount).toBe(1);
  const shortVec = (sqlite.query(
    `SELECT COUNT(*) AS n FROM chunk_vec
...
test("refresh-extraction: ordinal growth — more chunks on re-run leaves contiguous ordinals 0..N-1", async () => {
  ...
  await refreshExtraction(fakeCtx(sqlite, { text: "Tiny." }), { paper_id: "p5" });
  const tiny = (sqlite.query(
    "SELECT COUNT(*) AS n FROM paper_chunks WHERE paper_id='p5'",
  ).get() as { n: number }).n;
  expect(tiny).toBe(1);
  ...
  const long = (sqlite.query(
    "SELECT COUNT(*) AS n FROM paper_chunks WHERE paper_id='p5'",
  ).get() as { n: number }).n;
  expect(long).toBeGreaterThan(tiny);

  const vecCount = (sqlite.query(
    `SELECT COUNT(*) AS n FROM chunk_vec
  ...
  const maxOrd = (sqlite.query(
    "SELECT MAX(ordinal) AS m FROM paper_chunks WHERE paper_id='p5'",
  ).get() as { m: number }).m;
