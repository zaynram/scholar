...
// bun-launcher unification (2026-06-05).
//
...
// covers the GENERATED output. The launch model no longer branches per-OS — both
// targets run the same `bun ${ROOT}/bin/launch.mjs` shim — so what stays per-OS
// is the platform runtime root and the pre-warm hook (now also unified on the
// bun shim). Both are asserted below.
...
test('buildManifest(linux): bun launch.mjs + HOME runtime root + bun pre-warm', () => {
    ...
    expect(s.args).toEqual(['${CLAUDE_PLUGIN_ROOT}/bin/launch.mjs'])
    ...
    expect(hook.startsWith('bun ')).toBe(true)
})

test('buildManifest(win32): bun launch.mjs + USERPROFILE root + bun pre-warm', () => {
    const m = buildManifest(base, true) as any
    expect(m.bin).toBeUndefined()
    const s = m.mcpServers.scholar
    // Same cross-OS launcher as linux; only the runtime root differs per-OS.
    expect(s.command).toBe('bun')
    expect(s.args).toEqual(['${CLAUDE_PLUGIN_ROOT}/bin/launch.mjs'])
...
// the two vars launch.mjs + ensure-bun.ps1 read (CLAUDE_PLUGIN_ROOT := ${__dirname},
...
test('buildMcpbManifest: bun launch.mjs entry under type:binary, win32-only', () => {
    const m = buildMcpbManifest(base, '0.1.0') as any
    expect(m.server.type).toBe('binary') // bun is a binary on PATH
    expect(m.server.entry_point).toBe('bin/launch.mjs')
    expect(m.server.mcp_config.command).toBe('bun')
    expect(m.server.mcp_config.args).toEqual(['${__dirname}\\bin\\launch.mjs'])
...
test('buildMcpbManifest: env aliases the two vars the launcher reads (no launcher edit)', () => {
    ...
    // launch.mjs reads process.env.CLAUDE_PLUGIN_ROOT + CLAUDE_PLUGIN_DATA;
    // ensure-bun.ps1 reads $env:CLAUDE_PLUGIN_DATA. Aliasing both is what lets the
    // launcher run unmodified under Claude Desktop.
